...
# TVT1code block
scramble1_from = 0x29B6     # TVT1code
scramble1_to = 0x2AB6       # ELITE
scramble1_eor = 0xA5

# LOADcode block
scramble2_from = 0x1B29     # LOADcode
scramble2_to = 0x1BA3       # CATDcode
scramble2_eor = 0x18

# DIALS, SHIP_MISSILE and WORDS blocks
scramble3_from = 0x1D9F     # DIALS
scramble3_to = 0x299F       # OSBmod
scramble3_eor = 0xA5

# ELITE, ASOFT and CpASOFT blocks, plus padding to the end of the file
if release == 1 or release == 2:
    scramble4_from = 0x2AB6     # ELITE
    scramble4_to = 0x2E84       # End of ELITE4 file
    scramble4_eor = 0xA5
elif release == 3:
    scramble4_from = 0x2AB6     # ELITE
    scramble4_to = 0x2E84       # End of ELITE4 file (at PROT4)
    scramble4_eor = 0xA5

# Commander file checksum
tvt1_code = 0x29B6          # TVT1code
tvt1 = 0x1100               # TVT1
...
chk55ff = 0x1B76            # CHK55FF (address within ELITE4, as LOADcode is
...
# disk, with no decryption step in between, so in general this would need to
# checksum T.CODE's bytes exactly as the SC routine's EOR scramble leaves
# them on disk rather than the plain, unscrambled bytes - but this branch has
# T.CODE's own SC scramble step disabled entirely (see "Configuration
# variables for T.CODE" below), so T.CODE stays in its plain, unscrambled
# form both on disk and at runtime, and no scrambling is needed here either

tcode_data_block_scrambled = tcode_data_block
